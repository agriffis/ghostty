...
/**
 * Key event options for ghostty_key_event_set() and ghostty_key_event_get().
 *
 * Note that not all options can be used with both set and get. See the
 * associated documentation for each option for details.
 *
 * Be aware that some setters and getters are not straightforward
 * value assignments. Some setters or getters may perform additional
 * logic or transformations. Therefore, it may make sense to cache the 
 * result of a getter if you need to use it multiple times. If a getter
 * performs additional logic, it will be documented with the option.
 * If no notes are specified, the setter or getter is a straightforward
 * assignment and you probably don't need to cache the result.
 *
 * @ingroup key
 */
typedef enum {
     ...
     *
     * Set value type: const GhosttyKeyAction *
     * Get value type: GhosttyKeyAction *
     */
    GHOSTTY_KEY_EVENT_ACTION = 1,

    /**
     * Physical key code.
     *
     * Set value type: const GhosttyKey *
     * Get value type: GhosttyKey *
     */
    GHOSTTY_KEY_EVENT_KEY = 2,

    /**
     * Modifier keys bitmask.
     *
     * Set value type: const GhosttyMods *
     * Get value type: GhosttyMods *
     */
    GHOSTTY_KEY_EVENT_MODS = 3,

    /**
     * Consumed modifiers bitmask.
     *
     * Set value type: const GhosttyMods *
     * Get value type: GhosttyMods *
     */
    GHOSTTY_KEY_EVENT_CONSUMED_MODS = 4,

    /**
     * Whether the key event is part of a composition sequence.
     *
     * Set value type: const bool *
     * Get value type: bool *
     */
    GHOSTTY_KEY_EVENT_COMPOSING = 5,

    /**
     * UTF-8 text generated by the key event.
     *
     * Set value type: const char * (null-terminated string, or NULL for empty)
     * Get value type: const char ** (pointer to null-terminated string, or NULL for empty)
     * 
     * For set: The key event does NOT take ownership of the text pointer.
     * For get: The returned pointer is valid until the event is freed or UTF-8 is modified.
     */
    GHOSTTY_KEY_EVENT_UTF8 = 6,

    /**
     * Unshifted Unicode codepoint.
     *
     * Set value type: const uint32_t *
     * Get value type: uint32_t *
     */
    GHOSTTY_KEY_EVENT_UNSHIFTED_CODEPOINT = 7,
} GhosttyKeyEventOption;
...
/**
 * Set a property on a key event.
 *
 * The option parameter specifies what property to set, and the value 
 * parameter points to the data to set. The type of data pointed to by value 
 * depends on the option. These are all documented in the 
 * GhosttyKeyEventOption enum.
 *
 * This library uses this setter-style to ease ABI compatibility as the
 * key event structure may grow over time with additional properties.
 *
 * @param event The key event handle, must not be NULL
 * @param option The property to set
 * @param value Pointer to the value to set (type depends on option)
 *
 * @ingroup key
 */
void ghostty_key_event_set(GhosttyKeyEvent event, GhosttyKeyEventOption option, const void *value);

/**
 * Get a property from a key event.
 *
 * The option parameter specifies what property to get, and the value parameter 
 * points to where the result should be written. The type of data pointed to by 
 * value depends on the option. These are all documented in the 
 * GhosttyKeyEventOption enum.
 *
 * @param event The key event handle, must not be NULL
 * @param option The property to get
 * @param value Pointer to where the result should be written (type depends on option)
 *
 * @ingroup key
 */
void ghostty_key_event_get(GhosttyKeyEvent event, GhosttyKeyEventOption option, void *value);
