 ...
 *
 * Clipboard read requests (OSC 52 "?" and OSC 5522 reads) are never
 * forwarded: answering one would let any program running in the terminal
 * silently read the user's clipboard, and a VT state library has no way
 * to mediate that with user consent. OSC 5522 reads are answered with
 * the protocol's EPERM denial so clients degrade gracefully.
 *
 * @param terminal The terminal handle
 * @param userdata The userdata pointer set via GHOSTTY_TERMINAL_OPT_USERDATA
...
typedef enum GHOSTTY_ENUM_TYPED {
   ...
   * refused with ENOSYS). Clipboard read requests are never forwarded;
   * see GhosttyTerminalClipboardWriteFn.
   *
