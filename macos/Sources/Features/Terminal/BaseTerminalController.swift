...
extension BaseTerminalController {
    ...
    private func onConfirmClipboardRequest(
        ...
        // A clipboard read can originate from a surface that isn't focused.
        // Presenting its sheet immediately would bring that surface's window
        // forward and steal focus. Signal that it needs attention, retain the
        // request on the surface, and present it only after the surface gains focus.
        if case .osc_52_read = request.kind, !target.focused {
            if !target.bell {
                NotificationCenter.default.post(
