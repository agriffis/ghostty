...
/// non-empty.
///
/// If the `$EDITOR` environment variable is not set, this command will
...
pub fn run(alloc: Allocator) !u8 {
    ...
    // Get our EDITOR
    const get_env_ = try internal_os.getenv(alloc, "EDITOR");
    ...
    if (editor.len == 0) {
        try stderr.print(
            \\ The $EDITOR environment variable is not set or is empty. This
            \\ environment variable is required to edit the Ghostty configuration
            \\ via this CLI command.
            \\
            \\ Please set the $EDITOR environment variable to your preferred
            \\ terminal text editor and try again.
            \\
    ...
    try stderr.print(
        ...
        \\
        \\ This is usually due to the editor path not existing, invalid
